    ...
    session.headers.update(
        {
            ...
            "User-Agent": "cgv-api-watcher-subscriber-mailer/2.0",
        }
    )
...
def fetch_open_events(
    ...
    for theater in status.get("theaters") or []:
        ...
        for result in theater.get("results") or []:
            ...
            is_open = bool(result.get("imax_open"))
            movie_name = str(
                result.get("movie_name") or movie_keyword
            ).strip()
...
def alert_message(
    ...
    message["Subject"] = (
        f"[CGV IMAX 오픈] {date_label} {event.theater_name}"
    )
    message.set_content(
        "CGV IMAX 예매가 새로 열렸습니다.\n\n"
...
def main() -> int:
    ...
    try:
        ...
        alert_jobs: list[
            tuple[str, OpenEvent, list[Subscription]]
        ] = []
        ...
        for email, email_subscriptions in verified_by_email.items():
            for event in events:
                eligible = [
                    ...
                    if event_opened_after_verification(event, sub)
                    and (
                        sub.id,
                        event.event_key,
                        event.signature,
        ...
        with smtplib.SMTP_SSL(
            ...
            for sub in pending:
                smtp.send_message(
                    confirmation_message(sender, sub)
                )
                if call_rpc(
                    session,
                    base_url,
